
const typeIcons = {
    grass: "https://archives.bulbagarden.net/media/upload/0/08/Battrio_Grass_type.png",
    fire: "https://archives.bulbagarden.net/media/upload/8/8a/Battrio_Fire_type.png",
    water: "https://archives.bulbagarden.net/media/upload/2/29/Battrio_Water_type.png",
    bug: "https://archives.bulbagarden.net/media/upload/5/57/Battrio_Bug_type.png",
    flying: "https://archives.bulbagarden.net/media/upload/9/97/Battrio_Flying_type.png",
    dark: "https://archives.bulbagarden.net/media/upload/0/0f/Battrio_Dark_type.png",
    electric: "https://archives.bulbagarden.net/media/upload/0/02/Battrio_Electric_type.png",
    fighting: "https://archives.bulbagarden.net/media/upload/1/1b/Battrio_Fighting_type.png",
    ghost: "https://archives.bulbagarden.net/media/upload/2/28/Battrio_Ghost_type.png",
    normal: "https://archives.bulbagarden.net/media/upload/7/75/Battrio_Normal_type.png",
    poison: "https://archives.bulbagarden.net/media/upload/2/20/Battrio_Poison_type.png",
    psychic: "https://archives.bulbagarden.net/media/upload/6/61/Battrio_Psychic_type.png",
    rock: "https://archives.bulbagarden.net/media/upload/7/77/Battrio_Rock_type.png",
    steel: "https://archives.bulbagarden.net/media/upload/5/59/Battrio_Steel_type.png",
    dragon: "https://archives.bulbagarden.net/media/upload/7/70/Dragon_icon_SwSh.png",
    ice: "https://archives.bulbagarden.net/media/upload/0/01/Battrio_Ice_type.png",
    ground: "https://archives.bulbagarden.net/media/upload/f/f1/Battrio_Ground_type.png",
    fairy: "https://archives.bulbagarden.net/media/upload/c/c6/Fairy_icon_SwSh.png"
};

export default typeIcons;