...
import useMaxFavouritePokemon from '../hooks/useMaxFavouritePokemon';
...
const PokemonCard = ({ details }) => {
    ...
    const { favouritePokemon, addFavouritePokemon, removeFavouritePokemon, isFavouritePokemon } = useFavouritePokemon(FAVOURITE_POKEMON_KEY);
    const [showModal, closeModal, handleFavouriteToggle] = useMaxFavouritePokemon(6);

    const handleToggleFavourite = () => {
        if (isFavouritePokemon(id)) {
            removeFavouritePokemon(id);
        } else {
            if (!handleFavouriteToggle(favouritePokemon, id)) {
                return;
            }
        ...
        <div className="main">
            ...
            <div>
                <div className="stats-container" style={{ background: colorSwitcher(types[0].type.name) }}>
                    <FavoriteButton isFavourite={isFavouritePokemon(id)} onToggle={handleToggleFavourite} />
